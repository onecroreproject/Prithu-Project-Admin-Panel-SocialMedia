...
const CategorySelector = ({
    ...
    // Normalize category data and remove "God Quotes"
    const normalizedCategories = categories
        .filter(cat => {
            const name = (cat.categoriesName || cat.categoryName || cat.name || '').toLowerCase().trim();
    ...
    const matchedCategories = normalizedCategories.filter(c => 
        selectedIds.some(sid => {
            const sidStr = String(sid).toLowerCase().trim();
            const cidStr = c.id.toLowerCase().trim();
            const cnameStr = c.name.toLowerCase().trim();
            return sidStr === cidStr || 
                   sidStr === cnameStr || 
                   (sidStr === 'god' && (cnameStr === 'god' || cnameStr === 'gods' || (cnameStr.includes('god') && !cnameStr.includes('quote')))) || 
                   (sidStr.includes('special') && cnameStr.includes('special'));
        })
    );

    // If an ID was directly selected (e.g. quick select before list loads), preserve it in display
    const unmatchedIds = selectedIds.filter(sid => 
        !matchedCategories.some(mc => mc.id === String(sid) || mc.name.toLowerCase() === String(sid).toLowerCase())
    ).map(sid => ({
        id: String(sid),
        name: String(sid)
    }));

    const selectedCategories = [...matchedCategories, ...unmatchedIds];
    ...
    const toggleCategory = (id) => {
        if (singleSelect) {
            const isAlreadySelected = selectedIds.some(sid => 
                String(sid).toLowerCase().trim() === String(id).toLowerCase().trim()
            );
                        ...
                        {filteredCategories.length === 0 ? (
                            ...
                            filteredCategories.map(cat => {
                                const isSelected = selectedIds.some(sid => {
                                    const sidStr = String(sid).toLowerCase().trim();
                                    return sidStr === cat.id.toLowerCase() ||
                                           sidStr === cat.name.toLowerCase() ||
                                           (sidStr === 'god' && (cat.name.toLowerCase() === 'god' || cat.name.toLowerCase() === 'gods' || (cat.name.toLowerCase().includes('god') && !cat.name.toLowerCase().includes('quote')))) ||
                                           (sidStr.includes('special') && cat.name.toLowerCase().includes('special'));
                                });
                                return (
                                    <div
                                        key={cat.id}
